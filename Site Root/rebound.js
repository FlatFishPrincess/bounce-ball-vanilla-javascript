 ...
 * 
 * paddleLeft width => 16, right => 48px. if a ball hit on the edge of paddle, make the ball speed up
 * improving collision
 */
var ball;
var paddle;
...
// to keep track of user dragging paddle
var drag = false;
...
// make sure all devices supported
window.addEventListener('resize', init);

function init(){
  // store dom object in local area whenever possible!
...
function collisionY(){
  ...
  // speed up depending on how a ball hit on the edge of the paddle on both side
  if(ballTop > pHeight - 64){
    if(ballLeft >= paddleLeft + 16 && ballLeft < paddleLeft + 48){
      ...
      playSound(beepPaddle);
      return true;
    } else if(ballLeft > paddleLeft + 48 && ballLeft <= 64) {
      if(dx < 0)
        dx = -8;
      else
        dx = 8;
...
function gameOver(){;
  ...
  score.style.backgroundColor = 'rgba(128, 0, 0)';
